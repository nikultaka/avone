                ...
                $.ajax(settings).done(function(response) {
                    ...
                    if (response != '' && response != null) {
                        if (response.id != '' && response.id != null) {
                            ...
                            $.ajax({
                                url: BASE_URL + '/' + ADMIN + '/insert/deployment/db',
                                type: 'post',
                                data: {
                                    "_token": $("[name='_token']").val(),
                                    deploymentID: deploymentID,
                                    deploymentName: deploymentName,
                                    sizePerZoneElastic: sizePerZoneElastic,
                                    availabilityZonesElastic: availabilityZonesElastic,
                                    sizePerZoneKibana: sizePerZoneKibana,
                                    availabilityZonesKibana: availabilityZonesKibana,
                                    sizePerZoneApm: sizePerZoneApm,
                                    availabilityZonesApm: availabilityZonesApm,
                                    deploymentHdnID: deploymentHdnID,
                                },
        ...
        if (result.isConfirmed) {
            ...
            $.ajax(settings).done(function(response) {
                ...
                if (response != '' && response != null) {
                    if (response.id != '' && response.id != null) {
                        $.ajax({
                            url: BASE_URL + '/' + ADMIN + '/delete/deployment/db',
                            type: 'post',
                            data: {
                                "_token": $("[name='_token']").val(),
                                deploymentID: deploymentID,
